...
interface GuidedHelpBonusChallengeProps {
  persona: HelpPersona;
...
export function GuidedHelpBonusChallenge({
  persona,
  variant,
  animationsEnabled,
  onSolved,
  onSkip,
}: GuidedHelpBonusChallengeProps) {
    ...
    <article className={`card challenge-card guided-help-bonus-card ${persona.themeClass ?? ''}`}>
      ...
      {(type === 'text-input' || type === 'number-input') && (
        ...
        >
          ...
          <button type="button" className="btn" disabled={solved || !text.trim()} onClick={submit}>
            Check
          </button>
        </form>
      )}
      ...
      {solved && (
        <div className="feedback-card success" role="status">
          ...
          <p className="muted" style={{ marginTop: '0.5rem' }}>
            {persona.bonusSuccessNote}
          </p>
          {animationsEnabled && (
            <HelpMascot persona={persona} size={72} mood="happy" className="float" />
          )}
        </div>
      )}
      ...
      <div style={{ marginTop: '1rem' }}>
        <button type="button" className="btn btn-ghost" onClick={onSkip}>
          {solved ? persona.bonusDeclineLabel : 'Sla bonus over'}
        </button>
      </div>
    </article>
  );
